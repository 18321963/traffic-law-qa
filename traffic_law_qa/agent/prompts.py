...
`%(...)s` 的占位符由调用方补：规划轮的轮数上限、入口与审核轮的库内法规清单。
三处都**从运行时现取**，不写死在文本里。
"""

from __future__ import annotations

__all__ = ["AGENT_SYSTEM_PROMPT", "REFLECT_SYSTEM_PROMPT", "REGION_SYSTEM_PROMPT"]
...
4. 若审核消息里写了「下一轮用 <工具名> 查：……」，就照它指定的工具和那句话调用，
   **不要改写查询词、也不要换成另一个工具** —— 审核看得到你没看到的证据，它指到哪儿就打哪儿。
5. 查询里带上法规名片段（如「深圳」「智能网联汽车」）能提高对应法规的权重。
   检索的地区范围**入口已经按题目判好**（题面不提地点时也照判），你不需要操心；
   只有审核明确要你换一部法时才用 law_name 参数 —— 随手限定会把上位法一起排除掉。
6. 最多 %(max_steps)d 轮。"""
...
**最后指定下一轮该调哪个工具**（`next_tool`）—— 这是交给规划轮执行的指令，不是又一句描述：

- `get_article`：缺口是**某一条已知条号**的规定。条号都已经确定了还去检索是浪费 ——
  检索要拼召回，而精确取条直接就能拿到正文。
- `search_law`：缺口是**一类规定**、你还不知道它在第几条。这是默认。
- `none`：**不要再查了** —— 缺口在库外，或你已经判断再查也补不上。写 `none` 时
  `sufficient` 一并按 true 处理（你不必再单独填一遍）。
- 拿不准就留空：留空 = 「工具交给规划轮自己选」，与没有这个字段时行为相同。
...
{"sufficient": true 或 false, "retrievable": true 或 false,
 ...
 "next_tool": "search_law / get_article / none；拿不准留空"}
"""
