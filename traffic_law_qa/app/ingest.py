...
from ..indexing.parser import LawParser, reader_for
...
def read_material(data: bytes, display_name: str) -> tuple[list[Paragraph] | None, str]:
    try:
        paragraphs = read_paragraphs(data, display_name)
    except Exception as exc:  # noqa: BLE001
        return None, READ_FAILED_NOTE.format(error=exc)
    if not paragraphs:
...
def dry_run(
    ...
    digest = _sha1(data)
    paragraphs, note = read_material(data, name)
