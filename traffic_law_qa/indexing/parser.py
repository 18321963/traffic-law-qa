...
from .. import config
from ..contracts.disk import Article, Heading, LawDocument, Paragraph
from ..textutil import sha1_of
from .sources import READER_BY_SUFFIX, reader_for
...
class LawLibrary:
    ...
    def save_manifest(self, entries: list[dict]) -> dict:
        ...
        manifest = {
            ...
            "source_dir": config.SOURCE_DIR.relative_to(config.ROOT).as_posix(),
            "cross_check": False,
            "laws": entries,
        }
...
if __name__ == "__main__":
    raise SystemExit("已收口：请用 python -m traffic_law_qa.cli.build parse（清单见 README「入口」）")
