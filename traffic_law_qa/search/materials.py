...
from .. import config
from ..app.ingest import load_material_meta
from ..contracts.retrieval import MaterialPassage
from ..textutil import bigrams
from ..tools.render import LOG_MATERIAL
...
def load_materials(doc_ids: Sequence[str]) -> tuple[MaterialPassage, ...]:
    ...
    for doc_id in doc_ids:
        ...
        if not (meta_path.exists() and chunks_path.exists()):
            continue
        display_name = str(load_material_meta(doc_id).get("display_name") or doc_id)
...
def search_materials(
    ...
    grams = bigrams(_norm(query))
    scored: list[tuple[float, int, MaterialPassage]] = []
    for position, passage in enumerate(passages):
        overlap = len(grams & bigrams(_norm(passage.text)))
        if overlap:
            scored.append((overlap / max(1, len(grams)), position, passage))

    if not scored:
        head = passages[0]
        return [], f"{LOG_MATERIAL}0「{query}」｜命中 0 段\n" + EMPTY_NOTE.format(
            query=query,
            total=len(passages),
    ...
    lines = [
        f"{LOG_MATERIAL}{_label_no(hits[0].label)}「{query}」｜命中 {len(hits)} 段"
