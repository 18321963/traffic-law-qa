...
from .. import config
from ..contracts.retrieval import WebFinding
from ..tools.render import LOG_WEB
...
def _render(query: str, findings: list[WebFinding], start: int) -> str:
    if not findings:
        return f"{LOG_WEB}{start}「{query}」｜命中 0 条\n{EMPTY_NOTE}"
    return "\n".join(
        [f"{LOG_WEB}{start}「{query}」｜命中 {len(findings)} 条"] + [w.render() for w in findings]
    )
