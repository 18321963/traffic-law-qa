...
> 第三套题集 `data/eval_multihop.json`（63 道跨法多跳）在 §9，它另有自己的护栏与命令。
...
> 留下的 135 题**一道没动**——删的 304 条与它们没有交集。

⚠️ **但"题没动"不等于"数不会动"，下表动过两次，两次都不是题的错**：
...
|---|---|---|---|---|
| 稠密 + BM25 · bge-large-zh-v1.5（本地，**当前默认**） | 71.1% | 83.7% | 90.4% | 0.783 |
| 稠密 + BM25 · bge-large-zh-v1.5，不加查询前缀 | 69.6% | 83.7% | 87.4% | 0.767 |
| 稠密 + BM25 · text-embedding-v4（云端，旧） | 75.6% | 85.2% | 88.1% | 0.805 |
| 纯 BM25（与向量模型无关） | 74.1% | 83.7% | 89.6% | 0.795 |

**换成本地 bge 是一笔记在明面上的亏**：hit@1 −4.5pp、hit@3 −1.5pp、MRR −0.022，
只换回 hit@6 +2.3pp。换来的是**不花钱、不出网、快 2.7 倍**（135 题实测：本地混合
18.6s，云端混合 50.4s，纯 BM25 6.9s）。两样哪个更贵取决于部署场景，本项目选了本地；
**这个取舍可逆**，`.env` 换回云端那三行即可，两边的数都在上表里。

**查询指令前缀在 bge 上是必要用法，不是调参**：bge-zh 系列按「查询加前缀、段落不加」
训练，加上之后 hit@1 +1.5pp、hit@6 +3.0pp、MRR +0.016，hit@3 持平（上表第 1 行
...
**稠密通道在这套题上不赚，换成本地之后干脆是倒亏**：本地混合比纯 BM25 少 3.0pp
hit@1、少 0.013 MRR，只在 hit@6 上多 0.8pp（云端 v4 时是 +1.5pp hit@1 / +0.8pp
hit@3，至少不亏）。因为这些题的题面是从法条原文生成的，词面重合度高，BM25 本就
...
341 个测试，**全部离线可跑**（不连 Milvus、不发网络请求），`pytest -q` 约 9 秒：
...
| 臂 | 题集 | 结果 |
|---|---|---|
| 既有检索（同 §6） | 135 | hit@1 71.1% / hit@3 83.7% / hit@6 90.4% / MRR 0.783 |
| 规则取条 | 泄漏桶 208 | 触发 200/208（96.2%），其中就是 gold 199（**精确率 99.5%**）；回退 8 |
| 基线检索（同一批 208） | 泄漏桶 208 | hit@1 192/208（92.3%） hit@3 200/208（96.2%） hit@6 202/208（97.1%） |

**结论：这一臂比基线 hit@1 多 7 题（199 vs 192）。**
...
两个数仍不是同一件事：规则回退的 8 题里，基线有 7 题第 1 名就是 gold；
反过来规则也独得 15 题（基线第 1 名不是它）。合起来「触发就用规则、回退就走检索」
的上界 **206/208（99.0%）**。

> 口径提醒：**206 是部署口径** —— 规则一旦触发就不回退，所以"规则取错了、而基线
> 第 1 名是对的"那 1 题要算丢。换成**并集口径**（规则答对 ∪ 基线 top-k 内也对），
> top-1 是 207/208、top-3 与 top-6 都是 **208/208**：这批题里没有一条落在
> "规则也错、检索也够不着"的角落里（实测 0 条）。三个数都对，口径不同，别混着引用。
