...
| 用途 | 模型 | 端点 |
|---|---|---|
| 生成 / agent 规划 | `qwen-flash` | 阿里百炼 DashScope compatible-mode |
| 向量化 | `bge-large-zh-v1.5`（1024 维） | **本地 Ollama** `localhost:11434/v1` |

上表是**默认装出来是什么**（= `.env.example` 的值），不是「所有结论都由这一个模型跑出来」——
评测数字出自哪个模型，各节自己标了：§6 的单跳两臂出自 `qwen-flash`，§9 的多跳基线出自
`qwen-max`、换模型复跑出自 `qwen-turbo`。默认值只保证开箱能跑，免费额度见 `.env.example`。
...
401 个测试，**全部离线可跑**（不连 Milvus、不发网络请求），`pytest -q` 约 10 秒：
...
这两段**只在这里**（下面卷属主那段同此）。README 只留了「`--build` 不能省」那一句 ——
它是照着敲就可能踩的；这两屏排查细节对着门面上一放，就把「怎么装怎么跑」淹了。
真撞上了回头搜本文档。
...
> **这一节的结论有前提，读之前先看两条。** 其一：它测自 `qwen-max` **一个**模型，
> 而 2026-09-20 换模型复跑后，「同预算打平」这一格**不再成立**（见下面「换模型复跑」）。
> 其二：那一轮 agent 的审核节点**从未接上配置的模型**（见下面「两次接线事故」）。
> **别把它当与模型无关的结论读。**
...
所以在这个模型上：**agent 的增益 100% 来自「多轮检索」这个机制本身，第一次检索的选词
与 rag 没有任何差别。** 它没有「规划得更聪明」，只是「愿意再查一次」。这从反面印证了
`AGENT_MAX_STEPS` 默认取 2 而不是 3 那个取舍（`.env.example`）：值钱的是**有没有第二轮**，
不是轮数越多越好。

**这个「一样」比数字看上去更硬。** 按条配对算，两臂的 **Discordant 对为 0** ——
126 条 gold 上**逐条**命中完全相同（都中 59、都未中 67、各自独占 0），不是「总数恰好相等」。
原因也清楚：同一个检索器、同一份索引，唯一的差别是查询词谁写的（rag 用题面原文，
agent 用规划模型写的词），而这个模型恰好写得和题面一样好。

另外 agent「查到 68、引用 61」—— **找到不等于用上**，那 7 条它在最终答案里没引。

代价：检索次数 ×1.7（45 道跑了 2 轮、18 道 1 轮），外加每轮的规划与审核 LLM 调用。

#### 换模型复跑：同预算打平不再成立（50/63 题）

2026-09-20 实测，`LLM_MODEL=qwen-turbo`，前 50 题（第 51 题起撞上免费额度耗尽中断，
产物 `hop63_qwen-turbo.partial-50of63.json`；题序与 gold 已逐条核对与基线一致）：

| 臂 | 查到 gold | 引用 gold | 平均检索次数 |
|---|---|---|---|
| rag | 43/100（43.0%） | 43/100 | 1.0 |
| agent | 54/100（54.0%） | 48/100 | 1.6 |

同预算（两边都只看第一次检索），按条配对：

| 首次检索（按条配对） | rag 命中 | rag 未中 |
|---|---|---|
| **agent 命中** | 35 | 2 |
| **agent 未中** | **8** | 55 |

> **rag 单次 43/100  vs  agent 首次 37/100 —— 打平被打破，rag 反超 6 条。**

**但 50 题定不了论。** 10 条 Discordant（rag 独占 8、agent 独占 2），精确二项双尾
**p = 0.109**。方向明确、样本不够 —— 记成**提示**，不是结论。

这一格是**单变量**的，归因可以放心：首轮检索词只由规划模型写，而审核节点**在首轮检索
之后**才跑 —— 无论审核接没接线、配的哪个模型，都影响不到这一格。所以变化只能来自
生成/规划模型从 `qwen-max` 换成了 `qwen-turbo`。上面那句「换模型会不会改变结论，
没有测」的问题，答案现在是：**测了，方向变了，但不足以定论。**

全预算那两格（52→54 查到、46→48 引用）**不能这样归因** —— 它们同时含「生成模型换了」
和「审核终于真的接线了」两个变化。

#### 两次接线事故（都影响过这一节的数字）

**其一：`AGENT_REFLECT_*` 从未生效。** `AgentRunner.load()` 造了第二个客户端，但
`AgentRunner.graph()` 调 `build_graph()` 时**没把 `self.reflect_llm` 传进去**，
后者内部的 `reflect_llm = reflect_llm or llm` 于是落回规划轮那个模型。所以
**上表那一轮虽然配了审核模型，agent 臂实际是同一个模型在规划 + 审核**。2026-09-20
修复，并补了 `test_AgentRunner_把审核客户端接进图` 钉住这一跳。

> 值得记的是**为什么测试没拦住**：此前的 `test_审核真的走了另一个客户端` 测的正是这件事，
> 但它走 `run()` 辅助函数、而 `run()` **直接调 `build_graph(reflect_llm=...)`** ——
> 把 `AgentRunner.graph()` 整个绕过去了。测的是「`build_graph` 收不收」，不是
> 「`AgentRunner` 传不传」。bug 存在期间它一直是绿的。**配置项存在 ≠ 生效。**

**其二：免费额度按模型算，且正一个个耗尽。** 截至 2026-09-20，百炼上
`qwen3-max`（2026-09-19 耗尽）、`qwen-turbo`、`qwen-plus`、`qwen-max` 均已耗尽；
`qwen-flash`、`qwen3.6-flash`、`qwen3.7-flash` 仍可用。**上一版这里写的
「想复跑：`LLM_MODEL=qwen-max`，63 题约 16 分钟」已经跑不了了 —— `qwen-max` 现在也干了。**
...
- README 末尾的「进阶 / 再进阶」两组手记留**在那里**，不要搬走 —— 那是随手记，
  与本节的关系是「重复」不是「归档」：「微调 / 强化学习」= 上面「还没动的大件」，
  「更清晰的架构」≈ 上面那条目录结构

### 已经从 README 挪过来、且已经做完的

- README 太长太「AI」、不够清晰 → 已重写，162 行收到 97 行，删掉自证式的加粗与警告块。
  （2026-09-20 又把「它长什么样」换成实跑输出、删掉两处不实的引用格式，现 128 行）
