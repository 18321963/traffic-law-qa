...
SEARCH_LAW_TOOL: dict[str, Any] = {
    ...
    "function": {
        ...
        "description": (
            ...
            "需要多部法规才能回答的问题，请分多次检索，每次只问一件事，并换用不同关键词。"
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "检索词。尽量靠近法条用语，例如「醉酒驾驶 机动车 处罚」",
                },
                "law_name": {
                    "type": "string",
                    "description": (
...
# 条号：中文数字与阿拉伯数字都收。法条原文里一律是中文数字（实测 208/208），
...
def find_article(
    ...
    *,
    ...
    # 情形误判成歧义。实测这条让泄漏桶里 18 道题白白退回检索，其中十几道题面
    # 只提到了「实施条例」这一部法规。
